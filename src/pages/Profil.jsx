...
function App() {
  ...
  useEffect(() => {
    fetch("/data.json")
      ...
      .then((data) => {
        const userData = data.USER_MAIN_DATA.find((user) => user.id === parseInt(id));
        const userActivity = data.USER_ACTIVITY.find((activity) => activity.userId === parseInt(id));
        const userPerformance = data.USER_PERFORMANCE.find((performance) => performance.userId === parseInt(id));
        const userSessions = data.USER_AVERAGE_SESSIONS.find((sessions) => sessions.userId === parseInt(id));

        if (userData && userActivity && userPerformance && userSessions) {
          setUser({
            ...userData,
